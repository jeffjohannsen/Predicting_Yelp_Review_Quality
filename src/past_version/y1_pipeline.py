...
from y1_mongo2sql import save_to_postgres
...
class DataCleaningPipeline:
    ...
    def create_feature_sums(self):
        ...
        self.test["review_upvotes"] = self.test[review_positive_columns].sum(
            axis=1
        )
        self.test["user_upvotes"] = self.test[user_positive_columns].sum(
            axis=1
        )
        self.test["user_compliments"] = self.test[compliment_columns].sum(
            axis=1
        )
    ...
    def count_list_features(self):
        ...
        self.test[
            "restaurant_checkin_count"
        ] = self.test.restaurant_checkins.apply(counter)
        self.test.drop(
            labels=["user_friends", "restaurant_checkins"],
            axis=1,
            inplace=True,
        )
    ...
    def user_elite_feature(self):
        ...
        """
        self.test["user_elite_most_recent"] = self.test.user_elite.apply(
            most_recent_elite
        )
        self.test["user_most_recent_elite"] = pd.to_numeric(
            self.test["user_elite_most_recent"]
        )
        self.test["user_years_since_last_elite"] = (
            2020 - self.test["user_most_recent_elite"]
        )
        delete_elite = [
            "user_elite",
            "user_elite_most_recent",
    ...
    * Other Nan/Nulls avoided during sql inner joins
    ...
    def drop_nulls(self):
        ...
        self.test.drop(
            self.test[self.test["restaurant_price_range"] == "None"].index,
            inplace=True,
        )

    def convert_data_types(self):
        self.test["user_yelp_start"] = pd.to_datetime(
            self.test["user_yelping_since"]
        )
        self.test["restaurant_price"] = pd.to_numeric(
            self.test["restaurant_price_range"]
        )
    ...
    def difference_between_star_counts(self):
        self.test["review_stars_v_user_avg"] = (
            self.test["review_stars"] - self.test["user_average_stars_given"]
        )
        self.test["review_stars_v_restaurant_avg"] = (
            self.test["review_stars"] - self.test["restaurant_overall_stars"]
        )
        self.test["user_days_active_at_review_time"] = (
            self.test["review_date"] - self.test["user_yelp_start"]
        ).dt.days

    def create_target(self):
        database_creation = pd.to_datetime("2020-3-25 19:13:01")
        self.test["days_since_review"] = (
            database_creation - self.test.review_date
        ).dt.days
        self.test["TARGET_review_upvotes_time_adjusted"] = (
            self.test["review_upvotes"] / self.test["days_since_review"]
        )
        time_related_features_to_drop = [
            "days_since_review",
            "user_yelp_start",
            "review_date",
            "review_upvotes",
        ...
        self.test.drop(
            labels=time_related_features_to_drop, inplace=True, axis=1
        )

    def organize_features(self):
        features_ordered = [
            "TARGET_review_upvotes_time_adjusted",
            "review_stars",
            "review_stars_v_user_avg",
...
if __name__ == "__main__":
    ...
    for i in range(int(row_count / chunksize) + 1):
        ...
        data.test.to_sql(
            "model_data_1", con=engine, index=True, if_exists="append"
        )
        current_chunk_num += 1
        print(
            f"Save to Postgres Successful - Chunk Number: {current_chunk_num} of {total_chunks}"
        )
