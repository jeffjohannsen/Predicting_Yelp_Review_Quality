...
if __name__ == "__main__":
    train, test = load_data(
        "local", train_rec_count=5523992, test_rec_count=1382379
    )

    X_train, X_test, y_train, y_test = prep_data(
        train, test, feature_groups="top_features", scale_data=True
    )

    model_location = "/home/jeff/Documents/Data_Science_Projects/Yelp_Reviews/models/final_models/"

    # Classification Predictions
    clf_model = load(
        model_location + "log_reg_cv_ALL_top_features_tuned.joblib"
    )

    clf_predictions_train = clf_model.predict_proba(X_train)[:, 1]
    clf_predictions_test = clf_model.predict_proba(X_test)[:, 1]

    train.insert(1, "log_reg_pred_proba", clf_predictions_train)
    test.insert(1, "log_reg_pred_proba", clf_predictions_test)

    # Regression Predictions
    reg_model = load(model_location + "lin_reg_ALL_top_features.joblib")

    reg_predictions_train = reg_model.predict(X_train)
    reg_predictions_test = reg_model.predict(X_test)

    train.insert(1, "lin_reg_pred", reg_predictions_train)
    test.insert(1, "lin_reg_pred", reg_predictions_test)
