# TODO: Class and function documentation
...
class EDA_Prep:
    ...
    def target_time_discount(self, ufc_total, review_date):
        return (
            ufc_total / ((self.dataset_release_date - review_date).days)
        ) * 365
    ...
    def count_elite_td(self, user_elite, review_date):
        if user_elite in ["None", None, ""]:
            ...
            elites_pre_review = [
                elite for elite in split_elites if elite <= review_date.year
            ]
    ...
    def user_time_discount(
        self, count_feature, user_yelping_since, review_date
    ):
        return (
            count_feature
            / (
                self.dataset_release_date
                - (user_yelping_since - pd.Timedelta(days=1))
            ).days
        ) * ((review_date - (user_yelping_since - pd.Timedelta(days=1))).days)
    ...
    def business_time_discount(self, count_feature, review_date):
        return (
            count_feature
            / (self.dataset_release_date - self.yelp_founding_date).days
    ...
    def convert_to_datetime(self):
        self.df["review_date"] = pd.to_datetime(
            self.df["review_date"], unit="ms"
        )
        self.df["user_yelping_since"] = pd.to_datetime(
            self.df["user_yelping_since"]
        )

    def create_targets(self):
        # No discounting or scaling
        self.df["T1_REG_review_total_ufc"] = self.df[
            ["review_useful", "review_funny", "review_cool"]
        ].sum(axis=1)
        self.df["T2_CLS_ufc_>0"] = self.df["T1_REG_review_total_ufc"] > 0
        self.df["T3_CLS_ufc_level"] = self.df["T1_REG_review_total_ufc"].apply(
            self.usefulness_level
        )
        # Time discounted
        self.df["T4_REG_ufc_TD"] = self.df.apply(
            lambda x: self.target_time_discount(
                x.T1_REG_review_total_ufc, x.review_date
            ),
            axis=1,
        )
        self.df["T5_CLS_ufc_level_TD"] = self.df["T4_REG_ufc_TD"].apply(
            self.usefulness_level
        )
        ...
        self.df.drop(
            labels=["review_useful", "review_funny", "review_cool"],
            axis=1,
            inplace=True,
        )
    ...
    def combine_features(self):
        ...
        self.df["user_total_ufc"] = self.df[
            ["user_useful", "user_funny", "user_cool"]
        ].sum(axis=1)
        self.df.drop(
            labels=["user_useful", "user_funny", "user_cool"],
            axis=1,
            inplace=True,
        )

    def create_user_elite_basic_features(self):
        self.df["user_elite_count"] = self.df.user_elite.apply(
            self.count_elite
        )
        self.df[
    ...
    def create_user_elite_time_discounted(self):
        self.df["user_elite_count_TD"] = self.df.apply(
            lambda x: self.count_elite_td(x.user_elite, x.review_date), axis=1
        )
        self.df["user_years_since_most_recent_elite_TD"] = self.df.apply(
            lambda x: self.years_since_most_recent_elite_td(
                x.user_elite, x.review_date
            ),
            axis=1,
        )

    def time_discount_user_features(self):
        user_features_needing_time_discounting = [
            "user_total_ufc",
            "user_compliments",
            "user_review_count",
            "user_fans",
        ...
        for feature in user_features_needing_time_discounting:
            self.df[f"{feature}_TD"] = self.df.apply(
                lambda x: self.user_time_discount(
                    x[feature], x.user_yelping_since, x.review_date
                ),
                axis=1,
            )

    def time_discount_business_features(self):
        business_features_needing_time_discounting = [
            "business_review_count",
            "business_checkin_count",
        ]
        for feature in business_features_needing_time_discounting:
            self.df[f"{feature}_TD"] = self.df.apply(
                lambda x: self.business_time_discount(
                    x[feature], x.review_date
                ),
                axis=1,
            )
...
def data_prep_in_chunks_sql(chunksize, in_table, out_table, pipeline):
    ...
    row_count = int(
        pd.read_sql(f"SELECT COUNT(*) FROM {in_table}", engine).values
    )
    ...
    for i in range(int(row_count / chunksize) + 1):
        ...
        data.df.to_sql(out_table, con=engine, index=False, if_exists="append")
        current_chunk_num += 1
        ft = time.perf_counter()
        print(
            f"Chunk Number: {current_chunk_num} of {total_chunks} - "
...
def data_prep_in_chunks_pandas():
    ...
    row_count = int(
        pd.read_sql(f"SELECT COUNT(*) FROM {table_name}", engine).values
    )
    total_chunks = int(row_count / chunksize) + 1
    current_chunk_num = 0
    ...
    for chunk in pd.read_sql(query, engine, chunksize=chunksize):
        ...
        data.run_all()
        data.df.to_sql(
            "features_and_targets", con=engine, index=False, if_exists="append"
        )
        current_chunk_num += 1
        print(
            f"Chunk Number: {current_chunk_num} of {total_chunks} - "
