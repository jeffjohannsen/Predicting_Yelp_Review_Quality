...
class Base_Model_Process_Regression:
    ...
    def score_model(self, y_test):
        start = time.perf_counter()
        R2_score = r2_score(y_test, self.model_predictions)
        mae = mean_absolute_error(y_test, self.model_predictions)
        explained_variance = explained_variance_score(y_test, self.model_predictions)
        end = time.perf_counter()
...
if __name__ == "__main__":
    # Options
    model_naming_postfix = "_ALL_all_features"

    # Load Data
    train, test = load_data(
        ec2_or_local="ec2", train_rec_count=5523992, test_rec_count=1382379
    )

    # Feature Selection, Scaling, Train/Test Split
    X_train, X_test, y_train, y_test = prep_data(
        train, test, feature_groups="all", scale_data=True
    )

    # Linear Regression
    print(">>>>>>>>>>>>>>>> Linear Regression <<<<<<<<<<<<<<<<<<<<<<<<<")
    lin_reg_clf = LinearRegression(n_jobs=-1)
    lin_reg_process = Base_Model_Process_Regression(lin_reg_clf, "Linear Regression")
    lin_reg_process.fit_model(X_train, y_train)
    lin_reg_process.predict_model(X_test)
    lin_reg_process.score_model(y_test)
    lin_reg_process.save_model("lin_reg", model_naming_postfix)
