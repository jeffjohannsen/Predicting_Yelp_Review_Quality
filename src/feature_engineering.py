...
class Data_Finalization():
    ...
    def compute_star_deltas(self):
        ...
        self.df['review_stars_v_user_avg_sqr_diff'] = \
            (self.df['review_stars'] - self.df['user_avg_stars']) ** 2
        self.df['review_stars_v_business_avg_sqr_diff'] = \
            (self.df['review_stars'] - self.df['business_avg_stars']) ** 2

    # A lot of work that can still be done with how time with features.
    ...
    def compute_time_deltas(self):
        self.df['user_days_active_at_review_time'] = \
            (self.df['review_date'] - self.df['user_yelping_since']).dt.days
    ...
    def compute_user_feature_comparisons(self):
        ...
        self.df['user_ufc_per_years_yelping'] = \
            self.df['user_total_ufc'] \
            / (((self.dataset_release_date
                 - self.df['user_yelping_since']).dt.days)
                / 365)
        # fans per review
        self.df['user_fans_per_review'] = \
            self.df['user_fans'] / self.df['user_review_count']
        # fans per years_yelping
        self.df['user_fans_per_years_yelping'] = \
            self.df['user_fans'] / (((self.dataset_release_date
                                      - self.df['user_yelping_since']).dt.days)
                                    / 365)
        ...
        # total_ufc per years_yelpings
        self.df['user_ufc_per_years_yelping_TD'] = \
            self.df['user_total_ufc_TD'] / (((self.df['review_date']
                                            - (self.df['user_yelping_since']
                                            - pd.Timedelta(days=1))).dt.days)
                                            / 365)
        # fans per review
        self.df['user_fans_per_review_TD'] = \
            self.df['user_fans_TD'] / self.df['user_review_count_TD']
        # fans per years_yelping
        self.df['user_fans_per_years_yelping_TD'] = \
            self.df['user_fans_TD'] / (((self.df['review_date']
                                        - (self.df['user_yelping_since']
                                        - pd.Timedelta(days=1))).dt.days)
                                       / 365)
        # fans per review * total_ufc per review
    ...
    def delete_unusable_features(self):
        ...
        cause data leakage.
        """
        columns_to_drop = ['user_id', 'business_id', 'review_date',
    ...
    def organize_features(self):
        features_ordered = ['review_id', 'review_stars',
...
if __name__ == "__main__":
    data_prep_in_chunks_sql(100000, 'all_features',
                            'cleaned_data', EDA_Prep)
    data_prep_in_chunks_sql(100000, 'cleaned_data',
                            'non_nlp_model_data', Data_Finalization)
