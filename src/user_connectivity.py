# TODO: Build out on AWS with more computing power.
# TODO: OOP for metric calculation and visualization.
...
def create_edge_list(df):
    """
    Creates list of edges for networkx
    from original data friends lists.
    ...
    df["edge_list"] = [
        [(x, z) for z in y] for x, y in zip(df["user_id"], df["friends_list"])
    ]
    return df
...
def create_graph(num_rec):
    """
    Loads networkx graph form edge list.

    Args:
        num_rec (int): Number of records to use to build the graph.
...
if __name__ == "__main__":
    ...
    ft = time.perf_counter()
    print(
        f"Created the edge_list column with {num_records} records in {(ft - st):.4f} seconds"
    )
    st = time.perf_counter()
    Graph = create_graph(num_records)
    ft = time.perf_counter()
    print(
        f"Built the Graph from {num_records} records in {(ft - st):.4f} seconds"
    )
