...
class ModelPipeline:
    ...
    def prepare_data(self, target_name, features_to_ignore):
        ...
        self.features = self.data.drop(
            labels=features_to_ignore, axis=1, errors="ignore"
        )
    ...
    def scale_features(self):
        ...
        self.features = pd.DataFrame(
            scalar.fit_transform(self.features), columns=feature_names
        )
    ...
    def run_random_forest(self, params):
        forest = RandomForestClassifier(
            **params, random_state=5, oob_score=True, verbose=2
        )
        ...
        ftrimpt = pd.Series(
            feature_importances, index=self.features.columns
    ...
    def plot_roc_curves(self):
        ...
        forest = RandomForestClassifier(
            **forest_params, random_state=5, oob_score=True, verbose=2
        )
        forest.fit(self.X_train, self.y_train)
        ...
        fpr_forest, tpr_forest, _ = roc_curve(
            self.y_test, forest_predict_proba[:, [0]]
        )
...
def create_classification_data(df):
    data = df.copy()
    data["TARGET_review_has_upvotes"] = data[
...
def train_test_sql(df, sql_engine):
    ...
    train_data, test_data = train_test_split(
        data, test_size=0.20, random_state=5
    )
    train_data.to_sql(
        "model_data_cls_train",
        con=sql_engine,
        index=True,
        if_exists="replace",
        chunksize=50000,
...
if __name__ == "__main__":
    ...
    # Logistic
    log_params = dict(penalty="l2", C=1.0, solver="lbfgs", max_iter=100)
